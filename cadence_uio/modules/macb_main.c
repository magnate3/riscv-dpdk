...
static irqreturn_t macb_interrupt(int irq, void *dev_id)
{
	...
	status = queue_readl(queue, ISR);
        pr_err("status is : 0x%08x",status);
	if (unlikely(!status))
		return IRQ_NONE;
...
#if TEST_UIO
	int ret;
	pr_err("res->start = 0x%08lx\n", (long unsigned int)regs->start);
	pr_err("res->end = 0x%08lx\n", (long unsigned int)regs->end);
	the_uio_info.name = "cadence_uio";
	the_uio_info.version = "1.0";
        the_uio_info.mem[0].memtype = UIO_MEM_PHYS;
        the_uio_info.mem[0].addr = regs->start;
        the_uio_info.mem[0].size = resource_size(regs);
	the_uio_info.mem[0].name = "uio_driver_hw_resion";
        the_uio_info.mem[0].internal_addr = mem;
	ret = uio_register_device(&pdev->dev, &the_uio_info);
	if(ret != 0)
	{
	        pr_err("Could not register device \"led_uio\"...");
	}
#endif
